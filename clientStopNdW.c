...
int main() {
	...
	for(int i=0;i<10;i++) {
		flg=1;
		bzero(buff,sizeof(buff)); //pad buff
...
//Read message in socket var c_sock to buff  
        if(buff[strlen(buff)-1]!=i+'0'){          //acknowledgement not received at server,so discard
        printf("\nDiscarding packet\n");
       		i--;
		continue;
             }
		printf("Message received from server : %s \n",buff);
//buff contains information of message received from which server
